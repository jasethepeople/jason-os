...
import type { NamespacePermissions } from '@jason-os/shared';

function makePermissions(owner: string, encrypted: boolean): NamespacePermissions {
...
async function main(): Promise<void> {
  ...
  await test('burns namespace and removes all data', async () => {
    ...
    await ns.burn();
    assert.equal((ns as DataNamespaceImpl).isBurned(), true);
    await registry.destroy();
  });
