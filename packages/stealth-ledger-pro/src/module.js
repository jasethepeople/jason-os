...
function encryptAmount(amount, _key) {
...
export class StealthLedgerPro {
     ...
     */
    recalculateBalance() {
        this.state.balance = this.state.entries.reduce((sum, entry) => sum + entry.amount, 0);
        return this.state.balance;
    }
    /**
    ...
    emit(type, data) {
        if (this._bus &&
            ...
            if (b.emit && typeof b.emit === 'function') {
                b.emit({ type, data, source: 'stealth-ledger-pro' });
            }
        }
    }
}
...
// Factory
// ------------------------------------------------------------------
export function createStealthLedgerProModule(bus, config) {
