 ...
 * @returns plaintext
 */
export function decryptAES256GCM(ciphertext, iv, authTag, key) {
    if (key.length !== AES_KEY_SIZE) {
 ...
 */
export function deriveKeyPBKDF2(password, salt) {
    if (salt.length < 8) {
 ...
 */
export function secureRandom(bytes) {
 ...
 */
export function fingerprintKey(keyData) {
