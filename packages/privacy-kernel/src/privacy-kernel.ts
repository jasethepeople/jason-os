...
import type {
  PrivacyKernel as IPrivacyKernel,
  PrivacyTier,
  EncryptedBlob,
  KeyPair,
  KeyMaterial,
...
const GHOST_KEY_TTL_MS = 0; // immediate — GHOST keys are ephemeral
...
export class PrivacyKernel implements IPrivacyKernel {
  private _tier: PrivacyTier = 'PUBLIC';
  private readonly keychain: Keychain = new Keychain();
  private readonly tierHandlers: Set<(tier: PrivacyTier) => void> = new Set();
  ...
  async encrypt(data: Uint8Array, keyId: string): Promise<EncryptedBlob> {
  ...
  async decrypt(blob: EncryptedBlob, keyId: string): Promise<Uint8Array> {
  ...
  async generateSymmetricKey(): Promise<KeyMaterial> {
  ...
  async importKey(rawKey: Uint8Array, algorithm: string): Promise<CryptoKey> {
  ...
  private resolveKey(keyId: string): CryptoKey {
  ...
  private async importRawKey(rawKey: Uint8Array, algorithm: string): Promise<CryptoKey> {
    const crypto = globalThis.crypto;
    // Copy into a fresh ArrayBuffer-backed view: TS's Uint8Array<ArrayBufferLike>
    // isn't guaranteed to satisfy BufferSource (it could be backed by a
    // SharedArrayBuffer), so we make an explicit ArrayBuffer copy here.
  ...
  private async exportKeyToRaw(key: CryptoKey): Promise<Uint8Array> {
