...
export class SoftArchive {
  private items: Map<string, ArchiveItem> = new Map();
  private readonly config: ArchiveConfig;
  private lastArchivedAt: number | null = null;
  private bus: ArchiveEventBus | undefined;

  constructor(
    options: Partial<ArchiveConfig> & { bus?: ArchiveEventBus } = {},
  ) {
    this.config = {
  ...
  autoArchive(ageThresholdMs: number): string[] {
    if (!this.config.autoArchiveEnabled) return [];
  ...
  private calculateRelevance(item: ArchiveItem): number {
  ...
  private calculateRecencyBoost(archivedAt: number): number {
