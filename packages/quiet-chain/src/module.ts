...
export class QuietChain {
  ...
  private lastLinkAt: number | null = null;
  private accessPairs: Map<string, number> = new Map();
  ...
  getChain(rootId: string): ChainLink[] {
    if (!this.links.has(rootId)) return [];
  ...
  getRootLinks(): ChainLink[] {
    return this.roots
      ...
      .filter((link: ChainLink | undefined): link is ChainLink => link !== undefined)
      .map((link) => ({ ...link }));
  }
  ...
  traverseDepthFirst(startId: string): ChainTraversalResult[] {
    if (!this.links.has(startId)) return [];

    const result: ChainTraversalResult[] = [];
    const visited = new Set<string>();
  ...
  getState(): QuietChainState {
    const linksCopy = new Map<string, ChainLink>();
  ...
  reparentLink(id: string, newParentId: string | null): ChainLink | null {
    const link = this.links.get(id);
    if (!link) return null;
    if (newParentId !== null && !this.links.has(newParentId)) return null;
    if (newParentId === id) return null; // Cannot be own parent
