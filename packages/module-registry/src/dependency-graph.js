...
export class DependencyGraph {
    _graph = new Map();
    _reverseGraph = new Map();
    _nodes = new Map();
    _optionalDeps = new Map();
    /**
    ...
    detectCycles() {
        ...
        for (const node of allNodes) {
            if (!visited.has(node)) {
                ...
                // Re-mark all previously fully-processed nodes as visited
                const _previouslyVisited = new Set();
    ...
    topologicalSort() {
        ...
        while (queue.length > 0) {
            ...
            if (dependents) {
                for (const dependent of dependents) {
                    ...
                    if (!registeredNodes.has(dependent))
                        continue;
        ...
        if (result.length !== registeredNodes.size) {
            ...
            throw new ModuleError(`Cannot perform topological sort: cycle detected in dependency graph. Cycles: ${cycleStr}`, { cycles });
        }
        return result;
    }
    /**
     ...
     */
    getUnsatisfiedOptionalDependencies() {
