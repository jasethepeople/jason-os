...
export class ModuleRegistryImpl {
    ...
    register(manifest) {
        ...
        if (this._modules.has(validated.id)) {
            throw new ModuleError(`Module "${validated.id}" is already registered`, { moduleId: validated.id });
        }
        ...
        if (thisModuleUnsatisfied && thisModuleUnsatisfied.length > 0) {
            ...
            console.warn(`[ModuleRegistry] Module "${validated.id}" has unsatisfied optional dependencies: ${thisModuleUnsatisfied.join(', ')}`);
        }
    }
    /**
    ...
    listByCategory(category) {
        return [...this._modules.values()].filter((manifest) => manifest.category === category);
    }
    /**
    ...
    resolveDependencies(moduleId) {
        if (!this._modules.has(moduleId)) {
            throw new ModuleError(`Cannot resolve dependencies: module "${moduleId}" is not registered`, { moduleId });
        }
     ...
     */
    resolveDependents(moduleId) {
        if (!this._modules.has(moduleId)) {
            throw new ModuleError(`Cannot resolve dependents: module "${moduleId}" is not registered`, { moduleId });
        }
