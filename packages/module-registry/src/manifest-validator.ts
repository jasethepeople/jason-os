...
const VALID_CATEGORIES: readonly ModuleCategory[] = [
  'EMOTIONAL',
  'IDENTITY',
  'PRODUCTIVITY',
  'NAVIGATION',
...
export class ManifestValidator {
  private readonly _registeredIds: Set<string> = new Set();
  ...
  validate(manifest: unknown, registeredIds?: Set<string>): ModuleManifest {
    ...
    if (manifest === null || typeof manifest !== 'object') {
      throw new ValidationError(
        'Module manifest must be a non-null object',
    ...
    const optionalDependencies = this._extractOptionalStringArray(
      obj,
    ...
    const validated: ModuleManifest = {
      id,
      name,
      version,
      category,
  ...
  private _extractString(obj: Record<string, unknown>, field: string): string {
    const value = obj[field];
    if (typeof value !== 'string' || value.length === 0) {
      throw new ValidationError(
        `Module manifest must have a non-empty string "${field}"`,
  ...
  private _extractCategory(obj: Record<string, unknown>): ModuleCategory {
    const value = obj.category;
    if (typeof value !== 'string') {
      throw new ValidationError(
        'Module manifest "category" must be a string',
        { received: typeof value }
      );
    }

    if (!VALID_CATEGORIES.includes(value as ModuleCategory)) {
      throw new ValidationError(
        `Invalid module category "${value}". Must be one of: ${VALID_CATEGORIES.join(', ')}`,
        { category: value, validCategories: [...VALID_CATEGORIES] }
      );
    }
  ...
  private _extractStringArray(
    ...
    if (!Array.isArray(value)) {
      throw new ValidationError(
        `Module manifest "${field}" must be an array of strings`,
        { field, received: typeof value }
      );
    }
    ...
    for (const item of value) {
      if (typeof item !== 'string' || item.length === 0) {
        throw new ValidationError(
          `Module manifest "${field}" must contain only non-empty strings`,
          { field, item: typeof item }
        );
      }
    }
  ...
  private _extractPermissions(obj: Record<string, unknown>): Permission[] {
    const value = obj.permissions;
    if (!Array.isArray(value)) {
      throw new ValidationError(
        'Module manifest "permissions" must be an array',
        { received: typeof value }
      );
    }

    for (const perm of value) {
      if (typeof perm !== 'string' || !VALID_PERMISSIONS.includes(perm as Permission)) {
        throw new ValidationError(
          `Invalid permission "${String(perm)}". Must be one of: ${VALID_PERMISSIONS.join(', ')}`,
          { permission: perm, validPermissions: [...VALID_PERMISSIONS] }
        );
      }
    }
  ...
  private _extractEvents(obj: Record<string, unknown>): {
    emits: string[];
    listens: string[];
  } {
    ...
    if (value === null || typeof value !== 'object' || Array.isArray(value)) {
      throw new ValidationError(
        'Module manifest "events" must be an object with "emits" and "listens" arrays',
        { received: typeof value }
      );
    }
  ...
  private _extractUI(obj: Record<string, unknown>): {
    entryPoint: string;
    icon: string;
    ...
    if (value === null || typeof value !== 'object' || Array.isArray(value)) {
      throw new ValidationError(
        'Module manifest "ui" must be an object',
        { received: typeof value }
      );
    }
    ...
    const result: { entryPoint: string; icon: string; panelMode?: 'tabbed' | 'split' | 'overlay' | 'fullscreen' } = {
      entryPoint,
      icon,
    };
    ...
    if (ui.panelMode !== undefined) {
      ...
      if (
        ...
        throw new ValidationError(
          `Invalid panelMode "${String(mode)}". Must be one of: tabbed, split, overlay, fullscreen`,
          { panelMode: mode }
        );
      }
      result.panelMode = mode as 'tabbed' | 'split' | 'overlay' | 'fullscreen';
    }

    return result;
  }

  private _validateId(id: string, registeredIds?: Set<string>): void {
    const ids = registeredIds ?? this._registeredIds;
    if (ids.has(id)) {
      throw new ValidationError(
        `Duplicate module ID "${id}". A module with this ID is already registered.`,
        { moduleId: id }
      );
    }
  }
  ...
  private _validateVersion(version: string): void {
    if (!SEMVER_REGEX.test(version)) {
      throw new ValidationError(
        `Invalid version "${version}". Must be valid semver (MAJOR.MINOR.PATCH).`,
        { version }
      );
    }
  }

  private _validateEventTypes(events: {
    emits: string[];
    listens: string[];
  }): void {
    for (const type of events.emits) {
      if (type.length === 0) {
        throw new ValidationError(
          'Event type strings in "events.emits" must be non-empty',
          { eventType: type }
        );
      }
    }
    ...
    for (const type of events.listens) {
      if (type.length === 0) {
        throw new ValidationError(
          'Event type strings in "events.listens" must be non-empty',
          { eventType: type }
        );
      }
    }
