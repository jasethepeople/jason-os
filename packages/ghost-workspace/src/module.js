...
export const ghost_workspace_module = {
    ...
    category: 'privacy',
    version: '0.1.0',
    permissions: ['workspace:manage', 'events:emit', 'auth:verify'],
    description: 'Hidden workspace environment — privacy-focused workspace management with decoy mode',
};
// ------------------------------------------------------------------
...
export class GhostWorkspace {
     ...
     */
    switchWorkspace(workspaceId, authToken) {
    ...
    getActiveWorkspace() {
        if (!this.state.activeWorkspaceId)
            return null;
        const ws = this.state.workspaces.find((w) => w.id === this.state.activeWorkspaceId);
    ...
    emit(type, data) {
        if (this._bus &&
            ...
            if (b.emit && typeof b.emit === 'function') {
                b.emit({ type, data, source: 'ghost-workspace' });
            }
        }
    }
}
...
// Factory
// ------------------------------------------------------------------
export function createGhostWorkspaceModule(bus) {
